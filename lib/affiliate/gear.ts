...
export interface GearItem {
  ...
  en: GearText;
  /**
   * 파트너스 '간편 링크 만들기' 로 뽑아 온 링크(link.coupang.com/a/…).
   *
   * 가이드 26쪽이 정하는 순서는 [쿠팡 페이지 URL 복사 → 간편링크 만들기 → 생성된 URL
   * 사용] 이고, 검색결과 페이지도 그 대상이라고 25·26쪽에 명시돼 있다. 즉 우리가 만드는
   * 검색 주소는 대상이 맞지만, 규격대로라면 그 주소를 대시보드에 한 번 통과시켜야 한다.
   *
   * 그래서 이 칸이 채워져 있으면 그것을 그대로 쓰고(이미 태그가 박혀 있으므로 손대지
   * 않는다), 비어 있으면 lptag 를 직접 붙인 검색 주소로 넘어간다. 열여섯 개를 한 번에
   * 뽑아 오지 않아도 페이지가 도는 상태로 두기 위한 것이지, 직접 붙인 쪽이 같다는
   * 뜻은 아니다 - 채울 수 있으면 채우는 편이 안전하다.
   */
  coupangLink?: string;
}
