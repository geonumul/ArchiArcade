...
> 개인 학생은 영원히 무료. 과금은 조직(Org) 단위로만 발생한다.
> 광고는 화면 가장자리의 고정 배너 자리(스폰서 카트리지)까지만 허용한다 — 게임 위를 덮는 전면 광고,
> 소리·자동 재생, 탭을 가로채는 형태는 받지 않는다. 플레이를 방해하는 순간 관객이 사라지고,
> 관객이 없으면 그 자리도 팔리지 않는다.
